...
CREATE OR REPLACE TABLE sales_dwh.consumption.dim_promocodes (
    promotion_code_id NUMBER PRIMARY KEY,
    promotion_code TEXT,
    region TEXT,
    country TEXT,
    is_active TEXT(1)
...
-- fact_sales
CREATE OR REPLACE SEQUENCE sales_dwh.consumption.fact_sales_seq 
    START = 1 
    INCREMENT = 1;
...
ALTER TABLE sales_dwh.consumption.fact_sales 
    ...
    FOREIGN KEY (date_id) REFERENCES dim_dates (date_id) NOT ENFORCED;

ALTER TABLE sales_dwh.consumption.fact_sales 
    ADD CONSTRAINT fact_sales_customer_id_fk 
    FOREIGN KEY (customer_id) REFERENCES dim_customers (customer_id) NOT ENFORCED;

ALTER TABLE sales_dwh.consumption.fact_sales 
    ADD CONSTRAINT fact_sales_region_id_fk 
    FOREIGN KEY (region_id) REFERENCES dim_regions (region_id) NOT ENFORCED;

ALTER TABLE sales_dwh.consumption.fact_sales 
    ADD CONSTRAINT fact_sales_product_id_fk 
    FOREIGN KEY (product_id) REFERENCES dim_products (product_id) NOT ENFORCED;

ALTER TABLE sales_dwh.consumption.fact_sales 
    ADD CONSTRAINT fact_sales_promocode_id_fk 
    FOREIGN KEY (promocode_id) REFERENCES dim_promocodes (promocode_id) NOT ENFORCED;

ALTER TABLE sales_dwh.consumption.fact_sales 
    ADD CONSTRAINT fact_sales_payment_id_fk 
    FOREIGN KEY (payment_id) REFERENCES dim_payments (payment_id) NOT ENFORCED;
...
GRANT SELECT, INSERT ON TABLE sales_dwh.consumption.fact_sales TO ROLE sysadmin;
...
GRANT USAGE ON SEQUENCE sales_dwh.consumption.fact_sales_seq TO ROLE sysadmin;