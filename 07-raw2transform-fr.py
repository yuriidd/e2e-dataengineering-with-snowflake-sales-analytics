...
def main():
    ...
    sales_df = session.table("sales_dwh.raw.fr_sales_orders")
    ...
    country_sales_df = (
        filtered_df
        .with_column('country', lit('FR'))
        .with_column('region', lit('EU'))
    )
    ...
    df_to_load = merged_df.select(
        ...
        (col('final_price') / col('usd2eu')).alias('final_price_usd'),
        (col('tax_amount') / col('usd2eu')).alias('tax_amount_usd'), 
    )
    ...
    df_to_load.write.save_as_table(
        'sales_dwh.transform.fr_sales_orders', mode='append'
    )
