...
def main():
    ...
    sales_df = session.table("sales_dwh.raw.us_sales_orders")
    ...
    country_sales_df = (
        filtered_df
        .with_column('country', lit('US'))
        .with_column('region', lit('NA'))
    )
    ...
    df_to_load = merged_df.select(
        ...
        (col('final_price') / col('usd2usd')).alias('final_price_usd'),
        (col('tax_amount') / col('usd2usd')).alias('tax_amount_usd'), 
    )
    ...
    df_to_load.write.save_as_table(
        'sales_dwh.transform.us_sales_orders', mode='append'
    )
